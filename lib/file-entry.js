...
class FileEntryImpl extends Function {
    ...
    [Symbols.exec](...args) {
        return child_process_1.execFileSync(resolveExecName(this[Symbols.path]), resolveArgs(args));
    }
    [Symbols.execAsync](...args) {
        return exec_process_1.wrap(child_process_1.spawn(resolveExecName(this[Symbols.path]), resolveArgs(args), {
            windowsHide: true,
        }));
    }
...
switch (os_1.platform()) {
    case 'win32': {
        ...
        for (let i = startDrive; i <= endDrive; i++) {
            ...
            Object.defineProperty(exports.roots, `_${driveLetter}`, {
...
function resolveExecName(path) {
    try {
        if (!fs_1.existsSync(path))
            return which_1.sync(path_1.basename(path));
    }
    catch (_a) {
        // No luck
    }
...
function resolveAndWriteFile(path, data) {
    if (!utils_1.isThenable(data))
        return writeFile(path, data);
    Promise.resolve(data).then(resolvedData => writeFile(path, resolvedData));
}
function writeFile(path, data) {
    if (data == null)
        return void fs_1.writeFileSync(path, Buffer.allocUnsafe(0));
    if (data instanceof stream_1.Stream)
        return void data.pipe(fs_1.createWriteStream(path));
    if (typeof data === 'object' || typeof data === 'function') {
        const { stdout } = data;
        if (stdout instanceof stream_1.Stream)
            return void stdout.pipe(fs_1.createWriteStream(path));
    }
